...
extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        RenderUtil.drawRect(this.x, this.y, this.x + (float)this.width + 7.4f, this.y + (float)this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? OctoHack.colorManager.getColorWithAlpha(OctoHack.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : OctoHack.colorManager.getColorWithAlpha(OctoHack.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        OctoHack.textManager.drawStringWithShadow(" - " + this.setting.getName() + " " + ChatFormatting.RED + this.setting.getValue() + ChatFormatting.WHITE + " +", this.x + 2.3f, this.y - 4.0f - (float)OctoHackGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
    }
