...
extends Module {
    ...
    public Setting<Float> speed = this.register(new Setting<Object>("Speed", Float.valueOf(0.1f), Float.valueOf(0.0f), Float.valueOf(10.0f), v -> this.mode.getValue() == Mode.PACKET || this.mode.getValue() == Mode.DESCEND || this.mode.getValue() == Mode.VANILLA || this.mode.getValue() == Mode.DAMAGE, "The speed."));
    ...
    public Flight() {
        super("Flight", "Allows you to fly", Module.Category.MOVEMENT, true, false, false);
        this.setInstance();
    }
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        if (this.mode.getValue() == Mode.DAMAGE) {
            if (this.format.getValue() == Format.DAMAGE) {
                if (event.getStage() == 0) {
                    ...
                    if (Flight.mc.player.onGround) {
                        if (Flight.mc.player.isPotionActive(MobEffects.JUMP_BOOST)) {
                            motionY += (double)((float)(Flight.mc.player.getActivePotionEffect(MobEffects.JUMP_BOOST).getAmplifier() + 1) * 0.1f);
                        }
                        Flight.mc.player.motionY = motionY;
                        OctoHack.positionManager.setPlayerPosition(Flight.mc.player.posX, Flight.mc.player.motionY, Flight.mc.player.posZ);
                ...
                if (Flight.mc.gameSettings.keyBindJump.isKeyDown()) {
                    Flight.mc.player.motionY += (double)(this.speed.getValue().floatValue() / 2.0f);
                }
            ...
            if (this.format.getValue() == Format.NORMAL) {
                double d = Flight.mc.gameSettings.keyBindJump.isKeyDown() ? (double)this.speed.getValue().floatValue() : (Flight.mc.player.motionY = Flight.mc.gameSettings.keyBindSneak.isKeyDown() ? (double)(-this.speed.getValue().floatValue()) : 0.0);
                if (this.noKick.getValue().booleanValue() && Flight.mc.player.ticksExisted % 5 == 0) {
                    OctoHack.positionManager.setPlayerPosition(Flight.mc.player.posX, Flight.mc.player.posY - 0.03125, Flight.mc.player.posZ, true);
                }
        ...
        if (event.getStage() == 0) {
            ...
            if (this.mode.getValue() == Mode.VANILLA) {
                ...
                if (Flight.mc.gameSettings.keyBindJump.isKeyDown()) {
                    double d = this.noKick.getValue().booleanValue() ? (Flight.mc.player.ticksExisted % 20 == 0 ? (double)-0.04f : (double)this.speed.getValue().floatValue()) : (Flight.mc.player.motionY = (Flight.mc.player.motionY = Flight.mc.player.motionY + (double)this.speed.getValue().floatValue()));
                }
                if (Flight.mc.gameSettings.keyBindSneak.isKeyDown()) {
        ...
        if (event.getStage() == 0 && this.mode.getValue() == Mode.DAMAGE && this.format.getValue() == Format.DAMAGE) {
            ...
            if (this.level != 1 || Flight.mc.player.moveForward == 0.0f && Flight.mc.player.moveStrafing == 0.0f) {
                if (this.level == 2) {
                    ...
                    ++this.level;
                    difference = (Flight.mc.player.ticksExisted % 2 == 0 ? -0.05 : 0.1) * this.lastDist;
                ...
                this.level = 2;
                difference = Flight.mc.player.isPotionActive(MobEffects.SPEED) ? 1.86 : 2.05;
            }
    ...
    public void onPacketSend(PacketEvent.Send event) {
        if (event.getStage() == 0) {
            ...
            if (this.mode.getValue() == Mode.SPOOF) {
                ...
                List collisionBoxes = Flight.mc.player.world.getCollisionBoxes(Flight.mc.player, range);
    ...
    private void move(double x, double y, double z) {
        ...
        CPacketPlayer bounds = this.better.getValue() != false ? this.createBoundsPacket(x, y, z) : new CPacketPlayer.Position(Flight.mc.player.posX + x, 0.0, Flight.mc.player.posZ + z, Flight.mc.player.onGround);
        this.packets.add(bounds);
        Flight.mc.player.connection.sendPacket(bounds);
        ++this.teleportId;
