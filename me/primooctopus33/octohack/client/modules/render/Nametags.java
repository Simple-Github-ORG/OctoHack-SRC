...
extends Module {
    ...
    public Nametags() {
        super("Nametags", "Improves nametags and displays useful information", Module.Category.RENDER, false, false, false);
        this.setInstance();
    }
    ...
    public void onRender3D(Render3DEvent event) {
        if (!Nametags.fullNullCheck()) {
            for (EntityPlayer player : Nametags.mc.world.playerEntities) {
                if (player == null || player.equals(Nametags.mc.player) || !player.isEntityAlive() || player.isInvisible() && !this.invisibles.getValue().booleanValue() || this.onlyFov.getValue().booleanValue() && !RotationUtil.isInFov(player)) continue;
    ...
    private void renderNameTag(EntityPlayer player, double x, double y, double z, float partialTicks) {
        ...
        this.drawRect(-width - 2, -(Nametags.mc.fontRenderer.FONT_HEIGHT + 1), (float)width + 2.0f, 1.5f, 0x55000000);
        GlStateManager.enableAlpha();
        Nametags.mc.fontRenderer.drawStringWithShadow(this.getDisplayTag(player), -width, -(Nametags.mc.fontRenderer.FONT_HEIGHT - 1), this.getNameColor(player).getRGB());
    ...
    private int getDisplayColour(EntityPlayer player) {
        ...
        if (OctoHack.friendManager.isFriend(player)) {
            return -11157267;
        }
