...
import me.primooctopus33.octohack.util.StatusChamsUtil;
...
extends Module {
    ...
    public PopChams() {
        ...
        travel = this.register(new Setting<Boolean>("Travel", true));
        rL = this.register(new Setting<Integer>("Outline Red", 30, 0, 255));
        gL = this.register(new Setting<Integer>("Outline Green", 167, 0, 255));
        bL = this.register(new Setting<Integer>("Outline Blue", 255, 0, 255));
        aL = this.register(new Setting<Integer>("Outline Alpha", 255, 0, 255));
        rF = this.register(new Setting<Integer>("Fill Red", 30, 0, 255));
        gF = this.register(new Setting<Integer>("Fill Green", 167, 0, 255));
        bF = this.register(new Setting<Integer>("Fill Blue", 255, 0, 255));
    ...
    public void onUpdate(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof SPacketEntityStatus && (packet = (SPacketEntityStatus)event.getPacket()).getOpCode() == 35 && packet.getEntity(PopChams.mc.world) != null && (self.getValue().booleanValue() || packet.getEntity(PopChams.mc.world).getEntityId() != PopChams.mc.player.getEntityId())) {
            ...
            this.playerModel.field_78116_c.showModel = false;
            this.playerModel.field_78115_e.showModel = false;
            ...
            if (!onlyOneEsp.getValue().booleanValue()) {
                StatusChamsUtil statusChamsUtil = new StatusChamsUtil(this.player, this.playerModel, this.startTime, this.alphaFill, this.alphaLine);
            }
        }
    }
    ...
    public void onRenderWorld(RenderWorldLastEvent event) {
        if (onlyOneEsp.getValue().booleanValue()) {
            ...
            Color lineColorS = new Color(rL.getValue(), gL.getValue(), bL.getValue(), aL.getValue());
            Color fillColorS = new Color(rF.getValue(), gF.getValue(), bF.getValue(), aF.getValue());
