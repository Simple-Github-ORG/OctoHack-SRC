    ...
    public void onRender3D(Render3DEvent event) {
        ...
        for (UUID uuid : this.poses.keySet()) {
            ...
            for (int i = 1; i < this.poses.get(uuid).size(); ++i) {
                ...
                GL11.glColor4d((float)this.red.getValue().intValue() / 255.0f, (float)this.green.getValue().intValue() / 255.0f, (float)this.blue.getValue().intValue() / 255.0f, (float)this.alpha.getValue().intValue() / 255.0f);
                List<Vec3d> pos = this.poses.get(uuid);
                GL11.glVertex3d(pos.get((int)i).x - Trails.mc.getRenderManager().viewerPosX, pos.get((int)i).y - Trails.mc.getRenderManager().viewerPosY, pos.get((int)i).z - Trails.mc.getRenderManager().viewerPosZ);
                GL11.glVertex3d(pos.get((int)(i - 1)).x - Trails.mc.getRenderManager().viewerPosX, pos.get((int)(i - 1)).y - Trails.mc.getRenderManager().viewerPosY, pos.get((int)(i - 1)).z - Trails.mc.getRenderManager().viewerPosZ);
            }
            GL11.glEnd();
        }
        GL11.glEnable(3553);
