...
extends Module {
    ...
    public Setting slices = this.register(new Setting<Integer>("Slices", 100, 2, 100));
    ...
    public Trajectories() {
        super("Trajectories", "While holding a projectile, it will draw a line towards where your projectile will go", Module.Category.RENDER, false, false, false);
    }
            ...
            if (Trajectories.mc.gameSettings.thirdPersonView == 0 && (Trajectories.mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemBow || Trajectories.mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemFishingRod || Trajectories.mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemEnderPearl || Trajectories.mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemEgg || Trajectories.mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemSnowball || Trajectories.mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemExpBottle)) {
                ...
                while (!hasLanded && posY > 0.0) {
                    ...
                    for (Entity entity : entities) {
                        ...
                        if (!entity.canBeCollidedWith() || entity == Trajectories.mc.player || (possibleEntityLanding = (var8 = entity.getEntityBoundingBox().expand(0.3f, 0.3f, 0.3f)).calculateIntercept(present, future)) == null) continue;
                        hasLanded = true;
                    ...
                    this.drawLine3D((posX += motionX) - renderPosX, (posY += (motionY -= item instanceof ItemBow ? 0.05 : 0.03)) - renderPosY, (posZ += (motionZ *= (double)0.99f)) - renderPosZ);
                }
                if (landingPosition != null && landingPosition.typeOfHit == RayTraceResult.Type.BLOCK) {
                    ...
                    c.draw(this.size.getValue().floatValue(), this.innerSize.getValue().floatValue(), 0.0f, (Integer)this.slices.getValue(), 1);
                }
                this.disableGL3D();
                GL11.glPopMatrix();
            }
    ...
    private List<Entity> getEntitiesWithinAABB(AxisAlignedBB bb) {
        ...
        for (int x = chunkMinX; x <= chunkMaxX; ++x) {
            for (int z = chunkMinZ; z <= chunkMaxZ; ++z) {
                ...
                Trajectories.mc.world.getChunkFromChunkCoords(x, z).getEntitiesWithinAABBForEntity(Trajectories.mc.player, bb, list, null);
            }
        }
        return list;
    }
