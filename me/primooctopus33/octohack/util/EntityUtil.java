...
import me.primooctopus33.octohack.mixin.mixins.accessors.IEntityLivingBase;
...
implements Util {
    ...
    private static final BlockPos[] offsets = new BlockPos[]{new BlockPos(0, 0, -1), new BlockPos(1, 0, 0), new BlockPos(0, 0, 1), new BlockPos(-1, 0, 0)};
    ...
    public static EntityPlayer getTargetDouble(double range) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (!(EntityUtil.mc.player.getDistanceSq(player) < EntityUtil.mc.player.getDistanceSq(currentTarget))) continue;
            currentTarget = player;
        }
    ...
    public static List<Vec3d> getUnsafeBlocks(Entity entity, int height, boolean floor, boolean face) {
        return EntityUtil.getUnsafeBlocksFromVec3d(entity.getPositionVector().addVector(0.0, 0.125, 0.0), height, floor, face);
    }
    ...
    public static int getEntityPing(EntityPlayer player) {
        int ping = 0;
        try {
    ...
    public static double getDistPlayerToBlock(Entity p_Entity, BlockPos p_Pos) {
    ...
    private static boolean isValid(Entity entity, boolean players, boolean neutral, boolean friends, boolean hostile, boolean passive, double range) {
        ...
        if (entity instanceof EntityLivingBase && entity != EntityUtil.mc.player && entity.getDistanceSq(EntityUtil.mc.player) <= range * range) {
            if (entity instanceof EntityPlayer && players) {
                if (!friends) {
                    return !OctoHack.friendManager.isFriend((EntityPlayer)entity);
                }
                return true;
            }
    ...
    public static double getRange(Entity entity) {
        return EntityUtil.mc.player.getPositionVector().addVector(0.0, EntityUtil.mc.player.eyeHeight, 0.0).distanceTo(entity.getPositionVector().addVector(0.0, (double)entity.height / 2.0, 0.0));
    }
    ...
    public static EntityLivingBase getTarget(boolean players, boolean neutral, boolean friends, boolean hostile, boolean passive, double range, int mode) {
        ...
        if (mode == 0) {
            entity = EntityUtil.mc.world.loadedEntityList.stream().filter(entity1 -> EntityUtil.isValid(entity1, players, neutral, friends, hostile, passive, range)).min(Comparator.comparing(entity1 -> EntityUtil.mc.player.getPositionVector().squareDistanceTo(entity1.getPositionVector()))).orElse(null);
        } else if (mode == 1) {
            entity = EntityUtil.mc.world.loadedEntityList.stream().filter(entity1 -> EntityUtil.isValid(entity1, players, neutral, friends, hostile, passive, range)).map(entity1 -> (EntityLivingBase)entity1).min(Comparator.comparing((Function<EntityLivingBase, Float>)LambdaMetafactory.metafactory(null, null, null, (Ljava/lang/Object;)Ljava/lang/Object;, getHealth(), (Lnet/minecraft/entity/EntityLivingBase;)Ljava/lang/Float;)())).orElse(null);
        } else if (mode == 2) {
            entity = EntityUtil.mc.world.loadedEntityList.stream().filter(entity1 -> EntityUtil.isValid(entity1, players, neutral, friends, hostile, passive, range)).map(entity1 -> (EntityLivingBase)entity1).max(Comparator.comparing((Function<EntityLivingBase, Float>)LambdaMetafactory.metafactory(null, null, null, (Ljava/lang/Object;)Ljava/lang/Object;, getHealth(), (Lnet/minecraft/entity/EntityLivingBase;)Ljava/lang/Float;)())).orElse(null);
        }
        return entity;
    }
    ...
    public static double getDistance(double p_X, double p_Y, double p_Z, double x, double y, double z) {
        ...
        double d1 = p_Y - y;
        double d2 = p_Z - z;
        return MathHelper.sqrt((double)(d0 * d0 + d1 * d1 + d2 * d2));
    }
    ...
    public static boolean isMobAggressive(Entity entity) {
        if (entity instanceof EntityPigZombie) {
            ...
            if (entity instanceof EntityWolf) {
                return ((EntityWolf)((Object)entity)).isAngry() && !EntityUtil.mc.player.equals(((EntityWolf)((Object)entity)).func_70902_q());
            }
    ...
    public static double[] calcLooking(double px, double py, double pz, EntityPlayer me) {
    ...
    public static void swingArmNoPacket(EnumHand hand, EntityLivingBase entity) {
        ...
        if (!entity.isSwingInProgress || entity.swingProgressInt >= ((IEntityLivingBase)((Object)entity)).getArmSwingAnimationEnd() / 2 || entity.swingProgressInt < 0) {
    ...
    public static boolean isCrystalAtFeet(EntityEnderCrystal crystal, double range) {
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            if (EntityUtil.mc.player.getDistanceSq(player) > range * range || OctoHack.friendManager.isFriend(player)) continue;
    ...
    public static List<Vec3d> getUntrappedBlocksExtended(int extension, EntityPlayer player, boolean antiScaffold, boolean antiStep, boolean legs, boolean platform, boolean antiDrop, boolean raytrace, boolean noScaffoldExtend, boolean face) {
        ...
        if (extension == 1) {
            ...
            for (Vec3d vec3d : MathUtil.getBlockBlocks(player)) {
                ...
                placeTargets.addAll(EntityUtil.targets(vec3d, !noScaffoldExtend, antiStep, legs, platform, antiDrop, raytrace));
                ++extend;
            }
        }
    ...
    public static List<EntityPlayer> getNearbyPlayers(double d) {
        ...
        List<EntityPlayer> list = EntityUtil.mc.world.playerEntities.stream().filter(entityPlayer -> EntityUtil.mc.player != entityPlayer).filter(entityPlayer -> (double)EntityUtil.mc.player.getDistance((Entity)entityPlayer) <= d).filter(entityPlayer -> !(EntityUtil.getHealth(entityPlayer) < 0.0f)).collect(Collectors.toList());
        list.removeIf(entityPlayer -> OctoHack.friendManager.isFriend(entityPlayer.getName()));
        return list;
    }
    ...
    public static ArrayList<PairUtil<EntityPlayer, ArrayList<BlockPos>>> getCityablePlayers() {
        ...
        for (EntityPlayer entity : Objects.requireNonNull(EntityUtil.getNearbyPlayers(6.0)).stream().filter(entityPlayer -> !OctoHack.friendManager.isFriend((EntityPlayer)entityPlayer)).collect(Collectors.toList())) {
            ArrayList<BlockPos> arrayList2 = new ArrayList<BlockPos>();
            for (int i = 0; i < 4; ++i) {
                BlockPos blockPos = EntityUtil.getPositionVectors(entity, offsets[i]);
                if (EntityUtil.mc.world.getBlockState(blockPos).getBlock() != Blocks.OBSIDIAN) continue;
                boolean bl = false;
                switch (i) {
                    case 0: {
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals(EntityUtil.mc.player) || entity instanceof EntityPlayer && OctoHack.friendManager.isFriend(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer && colorFriends && OctoHack.friendManager.isFriend((EntityPlayer)entity)) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (OctoHack.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB.toString() + " \u00c2\u00a7f0", (int)EntityUtil.mc.player.getDistance(player));
