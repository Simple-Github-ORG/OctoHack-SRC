            ...
            case BAD_LOAD: {
                ...
                while (ia.hasNext() && ib.hasNext()) {
                    ...
                    if (na instanceof VarInsnNode && nb instanceof VarInsnNode) {
                        ...
                        Type ta = va.fd_int_2 > 0 ? argsa[va.fd_int_2 - 1] : target;
                        Type tb = vb.fd_int_2 > 0 ? argsb[vb.fd_int_2 - 1] : target;
                        printer.kv("Target " + index, "%8s %-2d %s", Bytecode.getOpcodeName(va), va.fd_int_2, ta);
                        printer.kv("Incoming " + index, "%8s %-2d %s", Bytecode.getOpcodeName(vb), vb.fd_int_2, tb);
                        if (ta.equals(tb)) {
